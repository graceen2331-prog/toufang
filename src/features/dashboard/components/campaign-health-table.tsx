...
export function CampaignHealthTable({
  campaigns,
  isLoading,
  isError,
  error,
  ...
  return (
    <Card className="rounded-lg border-0 bg-card/95 py-0 shadow-[0_14px_40px_oklch(0.24_0.03_250_/_0.08)] ring-1 ring-foreground/8">
      <CardHeader className="flex flex-row items-center justify-between gap-3 border-b px-5 py-4">
        <div>
          <CardTitle className="text-base font-semibold">近期 Campaign 健康度</CardTitle>
          <p className="mt-1 text-xs text-muted-foreground">按最近加载顺序展示 5 个 Campaign</p>
        </div>
        <Link
          href="/campaigns"
      ...
      <CardContent className="p-0">
        <AsyncBoundary
          ...
          emptyTitle="暂无 Campaign 数据"
          emptyHint="创建 Campaign 后会在这里显示近期健康状态"
        >
